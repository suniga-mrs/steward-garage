...
namespace Steward.WheelBox.Infrastructure.Persistence.Interceptors
{
    public class AuditableEntitySaveChangesInterceptor : SaveChangesInterceptor
    {
        ...
        public void UpdateEntities(DbContext? context)
        {
            ...
            foreach (var entry in context.ChangeTracker.Entries<BaseAuditableEntity>())
            {
                ...
                {
                    entry.Entity.CreatedBy = _currentUserService.UserGuid ?? "";
                    entry.Entity.DateCreated = _dateTime.Now;
                }

                if (entry.State == EntityState.Added || entry.State == EntityState.Modified || entry.HasChangedOwnedEntities())
                {
                    entry.Entity.LastModifiedBy = _currentUserService.UserGuid ?? "";
