...
<template>
    <BaseTopbarNavMenu :menu="topbarMenu">
        <template #navMenuItem="{ menu }">
            <li tabindex="1" v-for="(menuItem, index) in menu" :key="menuItem.route" :class="{
                        'parent-active': menuItem.name == currParentNavMenuItem?.name
                    }">
                <BaseLink class="topbar-menu-link" :to="{ name: menuItem.name }" active-class="active" data-parent="">
                    {{ menuItem.title }}
                </BaseLink>
            </li>
